document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentMode = '';
    const delayBetweenQuestions = 1000; // 答え表示から次の問題までの待ち時間 (1.5秒)
    ...
    const latestUpdate = {
        ...
        notes: [
            ...
            "・ゲームオーバー画面にリザルトを追加しました。",
            "・エンターキーを押したときに、正解なら緑、不正解なら赤のエフェクトを追加しました。",
            "・正誤判定後の次の問題への移行時間を増加させました。0.5<<<1.5",
            "",
    ...
    function resetGameUI() {
        ...
        inputElement.classList.remove('correct-effect', 'incorrect-effect');
    }
    ...
    function checkAnswer(answer) {
        ...
        const question = selectedQuestions[currentQuestionIndex];
        if (answer.toLowerCase().trim() === question.english.toLowerCase().trim()) {
            handleCorrectAnswer();
        } else {
            handleIncorrectAnswer(answer); // 入力された答えも渡す
        }
    }
    ...
    function handleCorrectAnswer() {
        correctAnswers++;
        score += 10;
        correctlyAnsweredQuestions.push(selectedQuestions[currentQuestionIndex]);
        inputElement.classList.add('correct-effect');
        updateUI();
        ...
        setTimeout(() => {
            ...
            inputElement.disabled = false;
            // 問題が最後まで終わったらゲームクリア
            if (currentQuestionIndex >= selectedQuestions.length && selectedQuestions.length === allQuestions[currentMode].length) {
                showGameClearScreen();
                return;
            }
            showQuestion();
    ...
    function handleIncorrectAnswer(userAnswer) {
        ...
        inputElement.classList.add('incorrect-effect');
    ...
    function showGameOverScreen() {
        ...
        let resultHtml = `
            <h3>全体結果</h3>
            <p><strong>全問題数:</strong> ${totalQuestions}問</p>
            <p><strong>正解数:</strong> ${correctAnswers}問</p>
            <p><strong>不正解数:</strong> ${incorrectAnswers}問</p>
        ...
        if (incorrectlyAnsweredQuestions.length > 0) {
            resultHtml += `
                ...
                <ul>
            `;
            incorrectlyAnsweredQuestions.forEach(q => {
                resultHtml += `<li>${q.japanese} → 正解: ${q.english} (あなたの答え: "${q.userAnswer}")</li>`;
            });
            resultHtml += `</ul>`;
        }
        ...
        if (correctlyAnsweredQuestions.length > 0) {
            ...
            correctlyAnsweredQuestions.forEach(q => {
                resultHtml += `<li>${q.japanese} → ${q.english}</li>`;
            });
            resultHtml += `</ul>`;
        }
        ...
        if (score > 100) {
            resultMessageElement.textContent = "素晴らしい！お見事です！";
        } else if (score > 50) {
            resultMessageElement.textContent = "よく頑張りました！あと一歩です！";
        } else {
            resultMessageElement.textContent = "残念！次は頑張りましょう！";
        }
    }
